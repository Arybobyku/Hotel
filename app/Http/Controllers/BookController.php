        ...
        $bookedRoomIds = DB::table('book_room_pivots')
            ->whereExists(function ($query) use ($startDate, $endDate) {
                $query->select(DB::raw(1))
                      ->from('books')
                      ->whereColumn('books.id', 'book_room_pivots.id_book')
                      ->where(function ($q) use ($startDate, $endDate) {
                          $q->whereDate('books.book_date', '<=', $endDate)
        ...
        $bookedRoomIds = DB::table('book_room_pivots')
            ->whereExists(function ($query) use ($startDate) {
                $query->select(DB::raw(1))
                      ->from('books')
                      ->whereColumn('books.id', 'book_room_pivots.id_book')
                      ->whereDate('books.book_date', $startDate);
            })
            ->pluck('id_room')
            ->toArray();
    } else {
        $bookedRoomIds = DB::table('book_room_pivots')
            ->whereExists(function ($query) use ($startDate) {
                $query->select(DB::raw(1))
                      ->from('books')
                      ->whereColumn('books.id', 'book_room_pivots.id_book')
                      ->whereDate('books.checkin', $startDate);
            })
            ->pluck('id_room')
            ->toArray();
    }

    // Filter hanya kamar yang tidak ada dalam daftar booked
    $availableRooms = $rooms->reject(fn($room) => in_array($room->id, $bookedRoomIds));
