...
class ShiftExport implements FromQuery, WithHeadings, WithStyles, ShouldAutoSize
{
     ...
     */
    use Exportable;
    protected $myId, $from, $to, $id_user, $tipee;

    function __construct($myId, $from, $to, $id_user, $tipee)
    {
        $this->myId = $myId;
        $this->from = $from;
        $this->to = $to;
    ...
    {
        if (!empty($this->from) && !empty($this->to) && !empty($this->id_user) && $this->tipee != NULL) {
