    ...
    <div class="w-full px-4">
        <div class="relative flex flex-col min-w-0 break-words w-full mb-2 shadow-lg rounded bg-white">
            ...
            <div class="block w-full overflow-x-auto p-8">
                <form method="post" action="/admin/hotel/{{$hotel->id }}/edit/{{$room->id}}" enctype="multipart/form-data">
                    @csrf
                    
                    @method('PUT')
                    <input name="id_hotel" value="{{ $room->id_hotel }}" hidden />
                    <div class="mb-6">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 ">Nama Rooms</label>
                        <input type="text" value="{{ $room->name }}" id="name" name="name"
                            class="form-control bg-gray-50 border border-gray-300 text-black text-sm rounded-lg block w-full p-2.5"
                            placeholder="" required>
                    </div>
                <div class="mb-6">
                <label class="block mb-2 text-sm font-medium text-gray-900" for="file_input">Upload image</label>
                <input value="{{ $room->image }}" class="block w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-pointer dark:placeholder-gray-400 @error('image') is-invalid @enderror" aria-describedby="file_input_help"  id="image" name="image" type="file">
